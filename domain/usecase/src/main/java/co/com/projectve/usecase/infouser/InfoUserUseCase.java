...
public class InfoUserUseCase {
    ...
    private final List<String> CREDIT_TYPES = Arrays.asList("Hipotecario", "Vehiculo", "Libre Inversion");
    ...
    private Mono<InfoUser> validateInfoUser(InfoUser infoUser) {
        return Mono.just(infoUser)
                ...
                .filter(iu -> iu.getDocumentNumber() != null && !iu.getDocumentNumber().isBlank())
                ...
                .filter(iu -> iu.getCreditTime() != null && !iu.getCreditTime().isBlank())
